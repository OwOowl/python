...
def insertData():
    ...
    if data1.replace(' ', '') != '' or data2.replace(' ', '') != '' or data3.replace(' ', '') != '' or data4.replace(' ', '') != '':
        try:
        ...
        con.commit()
        con.close()
        edt1.delete(0, len(data1))
        edt2.delete(0, len(data2))
        edt3.delete(0, len(data3))
        edt4.delete(0, len(data4))
    else:
        messagebox.showinfo('오류', '빈 칸을 입력하세요')
